...
test_list = ['fuck u ',
             ...
             'khate bahun',
             'radi chup muji',
             'tiktok chalauna ja muji',
             'kasto muji raixa',
             'Aakhe teri jhutno pe he kya'
...
async def on_ready():
    ...
    await client.change_presence(activity=discord.Activity(
        ...
        name=f' with your mom with {members}people '))
...
async def on_message(message):
    ...
    ch = client.get_channel(997498657949618206)
    ...
    if message.content.startswith('!rolldice!'):
        ...
        try:
            ...
            mama3011 = (txt3011[3]).replace('","error', '').replace('"', '').replace('none}',
                                                                                     'your dice dead lol, try again')

            data30111 = requests.get('https://api.catboys.com/dice').text

            txt30111 = str(data30111).split(':')

            mama30111 = (txt30111[3]).replace('","error', '').replace('"', '').replace('none}',
                                                                                       'your dice dead lol, try again')
    ...
    if message.content.startswith('!meeting!'):
        try:
            ...
            w2 = message.content.split('!')[3]
            await message.channel.send(
                'https://apimeme.com/meme?meme=Alien-Meeting-Suggestion&top=' + w1 + '&bottom=' + w2)
    ...
    if message.content.startswith('!hutao'):
        data30115 = requests.get('https://api.waifu.im/random/?selected_tags=hutao').text

        txt30115 = str(data30115).split(':')

        mama30115 = (txt30115[25]).replace('","preview_url', '').replace('"', '')

        await message.channel.send('https:' + mama30112)
        return
    if message.content.startswith("!play"):
        try:
            voice_clients = await message.author.voice.channel.connect()
...
client.run('MTAxMjMyNjY5Mzk2NjQ0NjYyMw.GW8N3W.4ROBCpAfahEnzSrV5cMg0fcyTUrV2xMkSksMiw')